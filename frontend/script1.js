...
async function handleSubscription(event) {
  ...
  // Send the subscription request to the backend
  try {
    const response = await fetch('http://localhost:5004/api/subscribe', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
signinBtn.addEventListener('click', async (event) => {
  ...
  const response = await fetch('http://localhost:5004/api/auth/signin', {
    method: 'POST',
...
signoutBtn.addEventListener('click', async () => {
  const response = await fetch('http://localhost:5004/api/auth/signout', {
    method: 'POST',
  });
